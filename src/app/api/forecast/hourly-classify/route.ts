import { NextResponse } from 'next/server';
import { supabase } from '@/lib/supabase';
import { execSync } from 'child_process';
import path from 'path';

const BP_PM25: number[][] = [[0,15,0,50],[15,35,50,100],[35,55,100,200],[55,150,200,300],[150,250,300,400],[250,350,400,500]];
const BP_PM10: number[][] = [[0,50,0,50],[50,150,50,100],[150,350,100,200],[350,420,200,300],[420,500,300,400],[500,600,400,500]];
const BP_CO: number[][] = [[0,5000,0,50],[5000,10000,50,100],[10000,17000,100,200],[17000,34000,200,300],[34000,46000,300,400],[46000,56000,400,500]];

const CAT_COLORS: Record<string, string> = {
    "Baik": "#10b981",
    "Sedang": "#3b82f6",
    "Tidak Sehat": "#f59e0b",
    "Sangat Tidak Sehat": "#ef4444",
    "Berbahaya": "#7c3aed",
};

function concToISPI(val: number, bp: number[][]): number {
    if (val <= 0) return 0;
    for (const [cl, ch, il, ih] of bp) {
        if (val <= ch) return il + (val - cl) / (ch - cl) * (ih - il);
    }
    return bp[bp.length - 1][3];
}

function getISPU(pm25: number, pm10: number, co: number): number {
    const ispis = [
        concToISPI(pm25, BP_PM25),
        concToISPI(pm10, BP_PM10),
        concToISPI(co, BP_CO),
    ];
    return Math.round(Math.max(...ispis) * 10) / 10;
}

function ispiToLabel(ispu: number): string {
    if (ispu <= 50) return "Baik";
    if (ispu <= 100) return "Sedang";
    if (ispu <= 200) return "Tidak Sehat";
    if (ispu <= 300) return "Sangat Tidak Sehat";
    return "Berbahaya";
}

function getDominant(pm25: number, pm10: number, co: number): string {
    const ispis: Record<string, number> = {
        "PM2.5": concToISPI(pm25, BP_PM25),
        "PM10": concToISPI(pm10, BP_PM10),
        "CO": concToISPI(co, BP_CO),
    };
    return Object.entries(ispis).reduce((a, b) => b[1] > a[1] ? b : a)[0];
}

export async function GET() {
    try {
        // Try to get predictions from database first
        const { data: dbPredictions, error: dbError } = await supabase
            .from('tb_predictions')
            .select('*')
            .order('target_at', { ascending: true })
            .limit(60);

        // Check if we have recent predictions (within last 10 minutes)
        let predictions: any[] = [];
        let method = "Database";

        if (!dbError && dbPredictions && dbPredictions.length > 0) {
            const latestPrediction = dbPredictions[0];
            const predictionTime = new Date(latestPrediction.target_at).getTime();
            const now = Date.now();
            const ageMinutes = (now - predictionTime) / (1000 * 60);

            if (ageMinutes <= 10) {
                // Use database predictions - they're fresh
                predictions = dbPredictions.map(p => ({
                    target_at: p.target_at,
                    pm25: p.pm25,
                    pm10: p.pm10,
                    co: p.co,
                    ispu: p.ispu,
                    category: p.category,
                    dominant: p.dominant,
                    color: p.color,
                    confidence: p.confidence,
                }));
                method = "Pre-computed XGBoost";
            } else {
                console.log('[/api/forecast/hourly-classify] Database predictions are stale, regenerating...');
            }
        }

        // If no predictions from DB, try to run prediction script
        if (predictions.length === 0) {
            const SCRIPT_PATH = path.resolve(process.cwd(), 'ml_model', 'predict_time_series.py');
            
            try {
                const output = execSync(`python "${SCRIPT_PATH}" --json`, {
                    timeout: 60000,
                    encoding: 'utf-8',
                    stdio: ['pipe', 'pipe', 'pipe'],
                    cwd: path.resolve(process.cwd()),
                });
                
                const parsed = JSON.parse(output.trim());
                if (parsed.forecast && parsed.forecast.length > 0) {
                    const forecastRows = parsed.forecast;
                    method = parsed.method || "XGBoost TimeSeries";
                    
                    // Classify each prediction
                    predictions = forecastRows.map(row => {
                        const ispu = getISPU(row.pm25, row.pm10, row.co);
                        return {
                            target_at: row.target_at,
                            pm25: Number(row.pm25?.toFixed(2)) || 0,
                            pm10: Number(row.pm10?.toFixed(2)) || 0,
                            co: Number(row.co?.toFixed(2)) || 0,
                            ispu: Number(ispu.toFixed(2)),
                            category: ispiToLabel(ispu),
                            dominant: getDominant(row.pm25, row.pm10, row.co),
                            color: CAT_COLORS[ispiToLabel(ispu)],
                            confidence: 0.85,
                        };
                    });
                }
            } catch (e: any) {
                console.error('[/api/forecast/hourly-classify] Prediction script failed:', e.message);
                
                // Fallback: simple prediction from latest sensor data
                const { data: rows } = await supabase
                    .from('tb_konsentrasi_gas')
                    .select('pm25_ugm3, pm10_ugm3, co_ugm3')
                    .order('created_at', { ascending: false })
                    .limit(1);

                if (rows && rows.length > 0) {
                    const pm25 = Number(rows[0].pm25_ugm3) || 0;
                    const pm10 = Number(rows[0].pm10_ugm3) || 0;
                    const co = Number(rows[0].co_ugm3) || 0;

                    for (let i = 0; i < 60; i++) {
                        const noise = (Math.random() - 0.5) * 5;
                        const targetAt = new Date(Date.now() + (i + 1) * 60000);
                        const p = Math.max(0, pm25 + noise);
                        const p10 = Math.max(0, pm10 + noise * 0.5);
                        const c = Math.max(0, co + noise * 10);
                        
                        const ispu = getISPU(p, p10, c);
                        predictions.push({
                            target_at: targetAt.toISOString(),
                            pm25: Number(p.toFixed(2)),
                            pm10: Number(p10.toFixed(2)),
                            co: Number(c.toFixed(2)),
                            ispu: Number(ispu.toFixed(2)),
                            category: ispiToLabel(ispu),
                            dominant: getDominant(p, p10, c),
                            color: CAT_COLORS[ispiToLabel(ispu)],
                            confidence: 0.5,
                        });
                    }
                    method = "Simple Fallback";
                }
            }
        }

        if (predictions.length === 0) {
            return NextResponse.json({ error: 'No forecast data available' }, { status: 400 });
        }

        const latest = predictions[59] || predictions[0];

        return NextResponse.json({
            forecast_at: new Date().toISOString(),
            forecast: predictions,
            classification: predictions,
            metadata: {
                latestISPU: Number(latest.ispu.toFixed(2)),
                latestCategory: latest.category,
                latestDominant: latest.dominant,
                latestColor: latest.color,
                method: method,
            },
        });
    } catch (err) {
        console.error('[/api/forecast/hourly-classify]', err);
        return NextResponse.json({ error: 'Internal error' }, { status: 500 });
    }
}