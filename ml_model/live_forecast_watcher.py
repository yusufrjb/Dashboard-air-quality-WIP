...
def run_forecast():
    ...
    try:
        # 1. Jalankan Peramalan (Time-Series)
        log.info("  -> Menjalankan predict_and_save.py")
        res1 = subprocess.run([sys.executable, "predict_and_save.py"], capture_output=True, text=True)
        if res1.stdout:
            for line in res1.stdout.splitlines():
                if "Berhasil" in line or "Saved" in line: log.info(f"     [Predict] {line}")
        
        # 2. Jalankan Regresi (Antar-Parameter)
        log.info("  -> Menjalankan regression_antar_parameter.py")
        res2 = subprocess.run([sys.executable, "regression_antar_parameter.py"], capture_output=True, text=True)
        if res2.stdout:
        ...
        if res1.returncode == 0 and res2.returncode == 0:
            ...
            log.warning(f"Ada pipa ML yang gagal (Codes: {res1.returncode}, {res2.returncode})")
            if res1.returncode != 0: log.error(f"Predict Error: {res1.stderr}")
            if res2.returncode != 0: log.error(f"Regress Error: {res2.stderr}")
